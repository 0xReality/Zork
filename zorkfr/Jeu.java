...
public class Jeu {
	...
	public void creerPieces() {
		...
		batimentF = new Piece("LE BAT INGE"); 
		


		// creation des Objets 
		ObjetZork stylo = new ObjetZork("Stylo", 12); 
		ObjetZork ordinateur = new ObjetZork("Ordinateur", 100); 
		ObjetZork chaise = new ObjetZork("Chaise", 150); 
		ObjetZork livre = new ObjetZork("Livre", 50); 
		...
		batimentF.setSorties(dehors, crous, null, batimentC);

		// le jeu commence dehors
		pieceCourante = dehors;
		
		//ajoute les objets dans la piéce 
		batimentF.ajouter(ordinateur); 
		salleTD.ajouter(stylo); 
		batimentC.ajouter(chaise); 
		taverne.ajouter(livre); 
	}
	...
	public boolean traiterCommande(Commande commande) {
		...
		else if(motCommande.equals("voir"))
	...
	public void voirObjet(Commande commmande) 
	{
		if(this.pieceCourante == batimentF)
		{
			stylo.getDescription();
		}
